...
class PieChart extends pChart {
	...
	function drawFlatPieGraph($Data, $DataDescription, $XPos, $YPos, $Radius = 100, $DrawLabels = PIE_NOLABEL, $SpliceDistance = 0, $Decimals = 0, $AllBlack = FALSE, ShadowProperties $shadowProperties = null) {
		...
		foreach ( $iValues as $Key => $Value ) {
			...
			/* Process pie slices */
			$this->processPieSlices($Angle, $SpliceRatio, $Value, $Radius, $XPos, $YPos, $XOffset, $YOffset, $Key, $color, $TopPlots, $shadowProperties);
	...
	private function processPieSlices(& $Angle, $SpliceRatio, $Value, $Radius, $XPos, $YPos, $XOffset, $YOffset, $Key, Color $color, array & $TopPlots, ShadowProperties $shadowProperties) {
		$lastPos = null;
